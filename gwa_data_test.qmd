---
title: "GWA data extration test"
format: html
---

## packages
```{r}
require(sf)
require(dplyr)
require(rnaturalearth)
require(ggplot2)
require(terra)
require(ggthemes)
require(fst)

source("aux_funct.R")
```


```{r}
# get UK polygon (ISO_A3 = "GBR")
uk <- rnaturalearth::ne_countries(scale = "large", country = c("United Kingdom","Ireland"), returnclass = "sf")

# project to metric CRS appropriate for Europe
uk_proj <- st_transform(uk, 3035)

# dissolve and buffer 300 km (300000 m)
uk_buffered <- st_buffer(st_union(uk_proj), dist = 250000, nQuadSegs = 100, allow_holes = TRUE)

# get bbox in WGS84 (lon/lat)
bbox_wgs84 <- st_bbox(st_transform(st_as_sfc(st_bbox(uk_buffered)), 4326))


```


## UK with buffer
```{r}
ggplot() +
  geom_sf(
    aes(geometry = geometry),
    data = uk_buffered |> st_transform(4326), 
    fill = "lightblue", color = "darkblue") +
  geom_sf(data = uk, fill = "lightgreen", color = "darkgreen") +
  coord_sf(xlim = c(bbox_wgs84["xmin"], bbox_wgs84["xmax"]),
           ylim = c(bbox_wgs84["ymin"], bbox_wgs84["ymax"]),
           datum = st_crs(4326)) +
  theme_minimal() +
#   coord_sf(datum = st_crs(4326)) + 
  ggtitle("UK with 250 km buffer")
```

## Load GWA raster data
```{r}
r <- terra::rast("~/Documents/GWA/GBR_wind-speed_10m.tif")
# names(r)
```

## extracting data frame
```{r}
varname <- names(r)[1]

# conver  to data.frame for plotting (removes NA cells)
df <- terra::as.data.frame(r, xy = TRUE, na.rm = TRUE)
if (ncol(df) >= 3) names(df)[3] <- "value" else stop("unexpected raster structure")

```
## visualize GWA data
```{r}

p_hist <- ggplot(df, aes(x = value)) +
  geom_histogram(bins = 60, fill = "grey30", color = "white") +
  theme_minimal() +
  labs(x = varname, y = "count")

ggsave(p_hist, filename = "fig/gwa_hist.png", width = 4, height = 3)
# example: extract raster value at UK centroid (project centroid to raster CRS first)
# uk_centroid <- st_centroid(st_union(uk))
# uk_centroid_proj <- st_transform(uk_centroid, crs = terra::crs(r))
# vals_at_centroid <- terra::extract(r, vect(uk_centroid_proj))

```


```{r}
df |> summary()

```

```{r}
ws_boxp <- df |> ggplot()+
    geom_boxplot(aes(x="10m",y=value))
ws_boxp
ggsave("fig/ws10_boxp.png", ws_boxp, width=3.5, height=3)

ws_densp <- df |> ggplot()+
    geom_density(aes(x=value), fill="lightblue", alpha=0.7)+
    labs(x="Wind speed at 10m", y="Density")+
    theme_minimal()

ws_densp
ggsave("fig/ws10_dens.png", ws_boxp, width=3.5, height=3)
```

```{r}
# map and histogram (ggplot objects)
p_map <- df |> slice(seq(1, n(), by = 5)) |>
  ggplot( aes(x = x, y = y, fill = value)) +
  geom_tile() +
  scale_fill_viridis_c(option = "magma", na.value = "transparent", name = varname) +
  coord_quickmap() +
  theme_minimal() +
  labs(title = paste0(varname, " (10 m)"), x = "lon", y = "lat")
p_map
```

```{r}
# convert buffered sf to terra vector in raster CRS
uk_buf_vect <- terra::vect(st_transform(uk_buffered, crs = terra::crs(r)))

# crop + mask to reduce compute and focus on region
r_crop <- terra::crop(r, uk_buf_vect)
r_mask <- terra::mask(r_crop, uk_buf_vect)

# quick numeric summaries (fast)
summary_simple <- terra::global(r_mask, fun = c("min", "mean", "sd", "max"), na.rm = TRUE)
summary_simple <- as.data.frame(summary_simple)
names(summary_simple) <- c("min", "mean", "sd", "max")

# quantiles (25%, 50%, 75%) via terra::global with a custom function
summary_quantiles <- terra::global(
  r_mask,
  fun = function(x, ...) {
    q <- stats::quantile(x, probs = c(0.25, 0.5, 0.75), na.rm = TRUE)
    c(q25 = q[1], median = q[2], q75 = q[3])
  }
)
summary_quantiles <- as.data.frame(summary_quantiles)

# aggregate (spatial downsampling) for quick plotting / map summary
# choose a factor; try 10, 20 or larger until plotting is responsive
agg_fact <- 20L
r_agg <- terra::aggregate(r, fact = agg_fact, fun = mean, na.rm = TRUE)

# aggregated df for ggplot (small)
df_agg <- terra::as.data.frame(r_agg, xy = TRUE, na.rm = TRUE)
# if (ncol(df_agg) >= 3) names(df_agg)[3] <- "value"

# efficient random sample of cells (no full as.data.frame on full raster)
# sample_size <- 5000L
# sampled_cells <- terra::spatSample(r_mask, size = sample_size, method = "regular", na.rm = TRUE, as.data.frame = TRUE)
# if (ncol(sampled_cells) >= 3) names(sampled_cells)[3] <- "value"

# # combine results into a single object for downstream use
# gwa_summary <- list(
#   masked_raster = r_mask,         # SpatRaster masked to UK+buffer
#   simple_stats = summary_simple,  # min, mean, sd, max
#   quantiles = summary_quantiles,  # 25%, median, 75%
#   aggregated_raster = r_agg,      # coarsened raster for plotting
#   aggregated_df = df_agg,         # small data.frame for ggplot
#   sampled = sampled_cells         # small random sample for exploration
# )

# gwa_summary
```

```{r}
r_crop |> head()
```
```{r}
# plot aggregated raster
p_agg_map <- df_agg |>
  ggplot( aes(x = x, y = y, fill = `GBR_wind-speed_10m`)) +
  geom_tile() +
  scale_fill_viridis_c(option = "magma", na.value = "transparent", name = "m/s") +
  coord_quickmap() +
  theme_minimal()+
  theme(legend.position = "bottom") +
  labs(x = "lon", y = "lat", title = "Aggregated wind speed at 10m")
  
p_agg_map
```

```{r}
p_agg_map
ggsave("fig/ws10_agg_map.png", p_agg_map, width=5, height=4)
```


GWA with UK + Ireland


```{r}

r_irl <- terra::rast("~/Documents/GWA/IRL_wind-speed_10m.tif")
agg_fact <- 20L
r_irl_agg <- terra::aggregate(r_irl, fact = agg_fact, fun = mean, na.rm = TRUE)
# aggregated df for ggplot (small)
df_irl_agg <- terra::as.data.frame(r_irl_agg, xy = TRUE, na.rm = TRUE)
```

```{r}
# plot aggregated raster
irl_agg_map <- df_irl_agg |>
  ggplot( aes(x = x, y = y, fill = `IRL_wind-speed_10m`)) +
  geom_tile() +
  scale_fill_viridis_c(option = "magma", na.value = "transparent", name = "m/s") +
  coord_quickmap() +
  theme_minimal()+
  theme(legend.position = "bottom") +
  labs(x = "lon", y = "lat", title = "Aggregated wind speed at 10m")
  
irl_agg_map
```

##test
```{r}
source("aux_funct.R")
dat0 <- get_gwa_data(
    "GBR", var = "wind-speed", height = 10,
    path = "~/Documents/GWA/")
```
### testing function to process GWA tif file
```{r}
agg_fact <- 5
r_agg <- terra::aggregate(dat0, fact = agg_fact, fun = mean, na.rm = TRUE)

df_agg <- terra::as.data.frame(r_agg, xy = TRUE, na.rm = TRUE)
var_name <- names(df_agg)[3]
meta_info <- strsplit(var_name, "_|m")[[1]]
df_agg <- df_agg |>
    mutate(
        country = meta_info[1],
        variable = meta_info[2],
        height = meta_info[3]
    ) |> 
    rename( value = !!var_name)
```

```{r}
source("aux_funct.R")
df_agg <- process_gwa_tif(
    dat0, agg_fact = 5
)
```
```{r}
df_agg |>
    ggplot( aes(x = x, y = y, fill = value)) +
  geom_tile() +
  scale_fill_viridis_c(option = "mako", na.value = "transparent", name = "m/s") +
  coord_quickmap() +
  theme_minimal()+
  theme(legend.position = "bottom") +
  labs(x = "lon", y = "lat", title = "Aggregated wind speed at 10m")
```

```{r}
var_name
meta_info <- strsplit(var_name, "_|m")[[1]]
meta_info
```



## get GWA ws two countries
### 10m
```{r}
source("aux_funct.R")
d_gb <- get_gwa_data(
    "GBR", var = "wind-speed", height = 10,
    path = "~/Documents/GWA/")
d_ir <- get_gwa_data(
    "IRL", var = "wind-speed", height = 10,
    path = "~/Documents/GWA/")

d_im <- get_gwa_data(
    "IMN", var = "wind-speed", height = 10,
    path = "~/Documents/GWA/")

# d_je<- get_gwa_data(
#     "JEY", var = "wind-speed", height = 10,
#     path = "~/Documents/GWA/")

# d_ge<- get_gwa_data(
#     "GGY", var = "wind-speed", height = 10,
#     path = "~/Documents/GWA/")

d_combined <- merge(d_gb, d_ir, d_im)#, d_je, d_ge)
# df_agg <- process_gwa_tif(
#     d_combined, agg_fact = 20
# ) 
# rm(df_agg)

```

```{r}
# df_gbr <- process_gwa_tif(
#   data = resample(d_gb, d_combined),
#   agg_fact = 20)
# df_irl <- process_gwa_tif(
#   data = resample(d_ir, d_combined),
#   agg_fact = 20)

df_all <- 
  lapply(
    list(d_gb, d_ir, d_im), 
    function(x) process_gwa_tif(
      data = resample(x, d_combined, threads = 8),
      agg_fact = 10)
  ) |> 
    bind_rows()
```


```{r}
df_all |>
    ggplot( aes(x = x, y = y, fill = value)) +
  geom_tile() +
  scale_fill_viridis_c(option = "mako", na.value = "transparent", name = "m/s") +
  coord_quickmap() +
  theme_minimal()+
  theme(legend.position = "bottom") +
  labs(x = "lon", y = "lat", title = "Wind speed at 10m")

ggsave("fig/ws_10m_uk_irl_im.png", width=5, height=7)
```


```{r}
source("aux_funct.R")
combined_gwa_data(
    countries = c("GBR", "IRL", "IMN"),
    var = "wind-speed",
    height = 10,
    path = "~/Documents/GWA/",
    show_fig = TRUE,
    save_df = TRUE,
    limits = c(0,10)
)
```

```{r}
combined_gwa_data(
    countries = c("GBR", "IRL", "IMN"),
    var = "wind-speed",
    height = 10,
    agg_fact = 1,
    path = "~/Documents/GWA/",
    show_fig = FALSE,
    save_df = TRUE,
    limits = c(0,10)
)
```

### 50m
```{r}

d_countries <- lapply(
    c("GBR", "IRL", "IMN"),
    \(x) get_gwa_data(
        country = x,
        var = "wind-speed", height = 50,
        path = "~/Documents/GWA/"))

d_combined_50m <- Reduce(merge, d_countries)

out_path <- "~/Documents/GWA/merged_wind_speed_50m.tif"

writeRaster(d_combined_50m, out_path, overwrite = TRUE)

```

```{r}

df_50m <- 
  lapply(
    d_countries, 
    function(x) process_gwa_tif(
      data = resample(x, d_combined_50m, threads = 8),
      agg_fact = 2)
  ) |> 
    bind_rows()

df_50m |>
    ggplot( aes(x = x, y = y, fill = value)) +
  geom_tile() +
  scale_fill_viridis_c(option = "mako", na.value = "transparent", name = "m/s") +
  coord_quickmap() +
  theme_minimal()+
  theme(legend.position = "bottom") +
  labs(x = "lon", y = "lat", title = "Wind speed at 50m")

ggsave("fig/ws_50m_merged.png", width=5, height=7)
```

```{r}
source("aux_funct.R")
combined_gwa_data(
    countries = c("GBR", "IRL", "IMN"),
    var = "wind-speed",
    height = 50,
    path = "~/Documents/GWA/",
    show_fig = TRUE,
    save_df = TRUE,
    limits = c(4,12)
)
```

```{r}
combined_gwa_data(
    countries = c("GBR", "IRL", "IMN"),
    var = "wind-speed",
    height = 50,
    agg_fact = 1,
    path = "~/Documents/GWA/",
    show_fig = FALSE,
    save_df = TRUE,
    limits = c(4,12)
)
```
### 100m

```{r}
source("aux_funct.R")
combined_gwa_data(
    countries = c("GBR", "IRL", "IMN"),
    var = "wind-speed",
    height = 100,
    path = "~/Documents/GWA/",
    show_fig = TRUE,
    save_df = TRUE,
    limits = c(5.5,12)
)


```

```{r}
source("aux_funct.R")
combined_gwa_data(
    countries = c("GBR", "IRL", "IMN"),
    var = "wind-speed",
    height = 100,
    agg_fact = 1,
    path = "~/Documents/GWA/",
    show_fig = FALSE,
    save_df = TRUE,
    limits = c(5.5,12)
)
```
### 150m

```{r}
combined_gwa_data(
    countries = c("GBR", "IRL", "IMN"),
    var = "wind-speed",
    height = 150,
    path = "~/Documents/GWA/",
    show_fig = TRUE,
    save_df = TRUE,
    limits = c(6.5,12.5)
)
```

```{r}
combined_gwa_data(
    countries = c("GBR", "IRL", "IMN"),
    var = "wind-speed",
    height = 150,
    agg_fact = 1,
    path = "~/Documents/GWA/",
    show_fig = FALSE,
    save_df = TRUE,
    limits = c(6.5,12.5)
)
```
### 200m


```{r}
combined_gwa_data(
    countries = c("GBR", "IRL", "IMN"),
    var = "wind-speed",
    height = 200,
    path = "~/Documents/GWA/",
    show_fig = TRUE,
    save_df = TRUE,
    limits = c(7,13.5)
)
```


```{r}
combined_gwa_data(
    countries = c("GBR", "IRL", "IMN"),
    var = "wind-speed",
    height = 200,
    agg_fact = 1,
    path = "~/Documents/GWA/",
    show_fig = FALSE,
    save_df = TRUE,
    limits = c(6.5,14)
)
```


## get GWA parameters 
### get weibull parameters

```{r}
source("aux_funct.R")
heights <- c(10,50,100,150,200)
lapply(
  heights,
  \(h){
    combined_gwa_data(
      countries = c("GBR", "IRL", "IMN"),
      var = "combined-Weibull-A",
      height = h,
      agg_fact = 20,
      path = "~/Documents/GWA/",
      show_fig = TRUE,
      save_df = TRUE
    )
  }
)

lapply(
  heights,
  \(h){
    combined_gwa_data(
      countries = c("GBR", "IRL", "IMN"),
      var = "combined-Weibull-k",
      height = h,
      agg_fact = 20,
      path = "~/Documents/GWA/",
      show_fig = TRUE,
      save_df = TRUE
    )
  }
)

```


```{r}
test <- combined_gwa_data(
    countries = c("GBR", "IRL", "IMN"),
    var = "combined-Weibull-A",
    height = 50,
    agg_fact = 20,
    path = "~/Documents/GWA/",
    show_fig = TRUE,
    save_df = TRUE
)
```

```{r}
ref_catalog_2025 <- fread(
  file.path("data/ref_catalog_wind_2025.csv.gz")) 

test$plot +
  geom_sf(
    data = ref_catalog_2025 %>% st_as_sf(coords = c("x_coord","y_coord"), crs = 27700) %>% 
    st_transform(crs = 4326),
    inherit.aes = FALSE,
    col = "lightgray"
  )
```

