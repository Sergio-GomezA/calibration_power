...
wind.bmus %>% 
  write.csv(
    .,gzfile(file.path(path, "wind_bmu_2.csv.gz"))
  )
```
...
ref_catalog_2025 %>% 
    ggplot()+
    geom_histogram(aes(capacity_turb), fill = "darkblue")+
    facet_wrap(~tech_typ)+
    labs(x = "Turbine capacity")
ggsave(
...
ref_catalog_2025 %>% 
    ggplot()+
    geom_histogram(aes(n_turb), fill = "darkblue")+
    facet_wrap(~tech_typ)+
...
ref_catalog_2025 %>% 
    ggplot()+
    geom_histogram(aes(height_turb), fill = "darkblue")+
    facet_wrap(~tech_typ)+
...
# 🔧 Create a complete grid of all dates and types
...
ggplot(capacity_time_full, aes(x = operational_date, y = cum_capacity, fill = tech_typ)) +
