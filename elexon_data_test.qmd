...
ref_catalog_2025 %>% 
    ggplot()+
    geom_boxplot(aes(
        x= tech_typ,y = generationCapacity
    ))
```
...
```{r}
# Aggregate capacity by date and type
capacity_time <- ref_catalog_2025 %>%
  mutate(operational_date = as.Date(operational, format = "%d/%m/%Y")) %>%
...
ggplot(capacity_time, aes(x = operational_date, y = cum_capacity, fill = tech_typ)) +
  geom_area(alpha = 0.8, color = "white", linewidth = 0.3) +
  scale_fill_manual(values = c("Wind Onshore" = "#66c2a5", "Wind Offshore" = "#fc8d62")) +
  labs(
    title = "Cumulative Wind Power Capacity in the UK",
    x = "Operational Date",
    y = "Cumulative Capacity (MW)",
    fill = "Type"
  ) +
