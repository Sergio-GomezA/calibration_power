...
n.days <- n.days.space
n.days.before <- n.days.before.space
n.hours <- n.hours.space
t0 <- d0 - n.days.before
t1 <- d0 + n.days
th <- t1 + hours(n.hours)
...
lm_pred_fig_df <- lm_pred %>%
  ...
  summarise(
    ...
    norm_potential_orig = sum(norm_potential_orig * capacity) / sum(capacity),
    norm_power_est0 = sum(norm_power_est0 * capacity) / sum(capacity),
    .groups = "drop"
  ) %>%
  mutate(
...
cat(sprintf(
  "Time taken for spatial out-of-sample validation: %.2f %s\n",
  timediff,
  units(timediff)
))
