...
\eta^{(0)}_{i}
  &= \alpha_{\mathrm{bern}}
     + f_{\mathrm{bern}}\!\left(w_i;\,\mathbf{s}(i)\right), \\[4pt]
Z_i \mid \eta^{(0)}_{i} 
  &\sim \mathrm{Bernoulli}(p_i),
  \qquad 
  p_i = \operatorname{logit}^{-1}\!\left(\eta^{(0)}_{i}\right),
\\[10pt]
%%
&\textbf{Positive--output model (Beta)} \nonumber \\[4pt]
\eta^{(\beta)}_{i}
  &= \alpha_{\mathrm{pc}}
     + f_{\mathrm{pc}}\!\left(w_i;\,\mathbf{s}(i)\right), \\[4pt]
P_i^{\text{obs}} \mid \eta^{(\beta)}_{i}
  &\sim \mathrm{Beta}(\mu_i,\phi),
  \qquad
  \mu_i = \operatorname{logit}^{-1}\!\left(\eta^{(\beta)}_{i}\right),
\\[10pt]
%%
&\textbf{Pseudo--likelihood model (Gaussian)} \nonumber \\[4pt]
\tilde{P}_i \mid \eta^{(\beta)}_{i}
  &\sim \mathcal{N}\!\left(
      \eta^{(\beta)}_{i},
      \tau_{\mathrm{ps}}^{-1}
    \right),
  \qquad
    \tau_{\mathrm{ps}} = \texttt{precision or penalisation}.
\end{align}
...
- Beta model with RW2 based power curve (B-RW2)
- Beta model with 1D SPDE based power curve (B-SPDE)
- ZIB model with 1D SPDE based power curve (ZIB-SPDE)
...
::: {.column width="50%"}
\small Correlation for ERA5 uncalibrated estimate

\includegraphics[width=\linewidth,height=5cm,keepaspectratio]{../fig/power_est_corr_dist.pdf}
:::
...
\begin{itemize}
   \item Combine updated power curve model with calibration
   \item Benefits of including spatial correlation  
   \item Model validation against benchmarks
\end{itemize}
