...
pow_comp <- ref_catalog_2025 %>% 
    ...
    scale_color_manual(values = mypalette)+
    labs(x = "Generic power curve GWh", y = "Elexon GWh", col = "")+
    theme(legend.position = "bottom")
...
pow_comp <- ref_catalog_2025 %>% 
    ...
    annotate(
      ...
      x = 1400,
      y = 700, 
      label = paste0("y = ", round(fixef(mod1_t)[1], 2), " x"), color = "darkblue", hjust = 0) +
    scale_color_manual(values = mypalette)+
    labs(x = "Generic power curve GWh", y = "Elexon GWh", col = "")+
    theme(legend.position = "bottom")
pow_comp
...
ref_catalog_2025_ll <- ref_catalog_2025 %>%
    ...
    tooltip_text = paste0(
      ...
      "<br>Wind Speed: ", round(gwa100, 2), " m/s",
      "<br>Elexon: ", round(quantity / 1000, 2), " GWh",
      "<br>Generic PC: ", round(gen_est / 1000, 2), " GWh",
      "<br>Capacity: ", generationCapacity, " MW",
      "<br>Type: ", tech_typ
    )
  ) %>%
...
pow_comp_plotly <- df_keyed %>%
  ...
  add_trace(
    x = c(0, max(ref_catalog_2025_ll$gen_est) / 1000),
    y = c(0, max(ref_catalog_2025_ll$gen_est) / 1000),
    type = 'scatter',
    mode = 'lines',
  ...
  add_trace(
    x = c(0, max(ref_catalog_2025_ll$gen_est) / 1000),
    y = c(0, max(ref_catalog_2025_ll$gen_est) / 1000 * fixef(mod1_t)[1]),
    type = 'scatter',
    mode = 'lines',
    line = list(dash = 'dash', color = 'darkblue'),
    showlegend = FALSE, inherit = FALSE
  ) %>%
  layout(
    xaxis = list(title = "Generic power curve GWh"),
    yaxis = list(title = "Elexon GWh"),
    legend = list(orientation = "h", y = -0.2),
    annotations = list(
      list(x = 1000, y = 900, text = "y = x", showarrow = FALSE, xanchor='left', font=list(color='black')),
      list(x = 1400, y = 700, text = paste0("y = ", round(fixef(mod1_t)[1],2), " x"), showarrow = FALSE, xanchor='left', font=list(color='darkblue'))
    )
  )
...
combo <- subplot(pow_comp_plotly, map_p, nrows = 1, widths = c(0.6, 0.4))

combo <- combo %>%
  layout(
    # left scatterplot uses the left 60 % of the page
    xaxis = list(domain = c(0, 0.6), title = "Generic power curve GWh"),
    yaxis = list(domain = c(0, 1), title = "Elexon GWh"),
...
```{r}
require(htmlwidgets)
saveWidget(combo, "combo_plot.html", selfcontained = TRUE)
browseURL("combo_plot.html")

htmlwidgets::saveWidget(combo, "combo_plot.html")
```


### add coordinates in crs 4326
```{r}
ref_catalog_2025 <- fread(
  file.path("data/ref_catalog_wind_2025.csv.gz")
) %>%
